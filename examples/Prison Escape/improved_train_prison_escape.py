"""
Improved training version with mechanisms to avoid convergence
towards static defensive strategies.

This module implements improved Q-Learning agents with anti-static mechanisms
for the Prison Escape environment. It includes curriculum learning features,
diversity bonuses, and adaptive exploration.

Example:
    python improved_train_prison_escape.py

Author: Generated by GAMA-PettingZoo
Date: 2024
Version: 0.2.0
"""
...
class ImprovedSimpleAgent:
    """Improved Q-Learning agent with anti-static mechanisms.
    
    This class implements an agent using the Q-Learning algorithm
    with improvements to avoid static behaviors:
    - Penalties for repetitive actions
    - Exploration bonuses for diversity
    - Adaptive epsilon based on behavior
    - Action and position history tracking
    
    Args:
        agent_name (str): Agent name ('prisoner' or 'guard')
        grid_size (int): Game grid size (default: 7)
    """
    
    def __init__(self, agent_name: str, grid_size: int = 7) -> None:
        """Initialize the improved Q-Learning agent."""
        self.agent_name = agent_name
        self.grid_size = grid_size
        self.action_count = 4
        
        # Q-table with optimistic default values
        self.q_table = {}
        ...
        self.action_history = deque(maxlen=10)  # Last actions history
        self.position_history = deque(maxlen=20)  # Position history
        self.static_penalty = 0.01  # Penalty for staying static
        
        self.last_state = None
        self.last_action = None
        self.step_count = 0
        
    def get_state_key(self, observation: np.ndarray) -> tuple:
        """Convertit l'observation en clé d'état.
        
        Args:
            observation: Array numpy représentant l'observation
    ...
    def is_static_behavior(self) -> bool:
        """Détecte si l'agent a un comportement statique.
        
        Analyse l'historique des actions et positions pour détecter :
        - Répétition des mêmes actions
        - Confinement dans une zone restreinte
        
        Returns:
            True si comportement statique détecté, False sinon
        """
    ...
    def get_action(self, observation: np.ndarray, training: bool = True) -> int:
        ...
        Args:
            ...
            training: Mode entraînement (exploration) ou évaluation
            
        Returns:
            Action choisie (entier entre 0 et action_count-1)
        """
...
class ImprovedTrainer:
    ...
    This class orchestrates agent training with:
    - Curriculum learning adapté
    - Bonus de diversité pour les épisodes intéressants
    - Sauvegarde périodique des modèles et métriques
    - Statistiques détaillées de performance
    
    Args:
        experiment_path (str): Path to the .gaml experiment file
        save_dir (str): Répertoire de sauvegarde des modèles
    """
    
    def __init__(self, experiment_path: str, save_dir: str = "./improved_models") -> None:
        """Initialise le trainer amélioré."""
    ...
    def calculate_diversity_bonus(self, episode_length: int, winner: str = None) -> float:
        """Calcule un bonus basé sur la diversité du jeu.
        
        Args:
            episode_length: Nombre de pas dans l'épisode
            winner: Nom du gagnant ou None pour match nul
            
        Returns:
            Bonus de diversité (float)
        """
        # Constantes pour les bonus
        MIN_INTERESTING_LENGTH = 20
        MAX_INTERESTING_LENGTH = 80
        LENGTH_BONUS = 0.1
        WINNER_BONUS = 0.2
        
        bonus = 0.0
        
        # Bonus pour les épisodes de longueur intéressante
        if MIN_INTERESTING_LENGTH <= episode_length <= MAX_INTERESTING_LENGTH:
            bonus += LENGTH_BONUS
